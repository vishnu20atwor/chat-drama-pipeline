...
// The thumbnail text: the title, big, in a bright block. It is on screen the
// whole time — on a Short the viewer decides in the first second.
const Headline = ({skin, title, frame}) => {
  const {fps} = useVideoConfig();
  const s = spring({frame, fps, config: {damping: 16, stiffness: 160, mass: 0.8}});
  ...
  return (
    <div style={{position: 'absolute', top: HEADLINE_Y, left: 0, width: W, height: HEADLINE_H, display: 'flex', alignItems: 'center', padding: `0 ${PAD_X}px`}}>
      <div style={{background: skin.headlineBg, color: skin.headline, fontFamily: FONT, fontWeight: 800, fontSize: px, lineHeight: 1.12, padding: '18px 26px', borderRadius: 22, transform: `scale(${0.9 + 0.1 * s}) rotate(-1deg)`, opacity: s, maxWidth: W - PAD_X * 2, boxShadow: '0 12px 40px rgba(0,0,0,0.25)'}}>
        {title}
      </div>
    </div>
  );
...
export const Chat = ({content, audio}) => {
  ...
  // What is on screen at this frame, with the newest item growing in so the
  // older ones slide up instead of jumping.
  ...
  for (let k = 0; k < tl.items.length; k++) {
    ...
    const p = ease(frame, it.from, ENTER);
    let h = heights[k];
    let gap = 0;
    let showName = false;
    if (it.kind === 'msg') {
      gap = rows.length === 0 ? 0 : prevWho === it.who ? GAP : RUN_GAP;
      showName = group && it.side === 'in' && prevWho !== it.who;
      if (showName) h += NAME_H;
      prevWho = it.who;
      prevSide = it.side;
    } else gap = RUN_GAP;
    rows.push({kind: it.kind, item: it, h, gap, showName});
    total += (h + gap) * p;
  }
  const offset = Math.max(0, total - CHAT_H);

  // The twist beat: a punch-in and a flash on a [boom] bubble.
  const boom = tl.items.find((it) => it.boom && frame >= it.from && frame < it.from + 10);
  const punch = boom ? 1 + 0.05 * (1 - ease(frame, boom.from, 10)) : 1;
  ...
  return (
    <AbsoluteFill style={{background: skin.bg, fontFamily: FONT}}>
      ...
      <AbsoluteFill style={{transform: `scale(${punch})`, transformOrigin: '50% 45%'}}>
        <StatusBar skin={skin} clock={clockOf(content.id)} />
